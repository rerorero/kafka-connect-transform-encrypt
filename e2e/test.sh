...
docker-compose exec -T connect kafka-console-consumer --topic pageviews --bootstrap-server kafka:29092  --property print.key=true --max-messages 1 --from-beginning --timeout-ms 10000 | grep '"userid":"User_'
docker-compose exec -T connect kafka-console-consumer --topic pageviews --bootstrap-server kafka:29092  --property print.key=true --max-messages 1 --from-beginning --timeout-ms 10000 | grep '"pageid":"vault:v1'
